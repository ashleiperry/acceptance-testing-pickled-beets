...
feature "User checks a recipe's deliciousness", %(
  ...
  scenario "user submits a recipe name without 'pickled beets'" do
    visit '/'
    fill_in 'recipe_name', with: 'creamy chicken pasta'
    click_button 'Submit'
    expect(page).to_not have_content("is a delicious recipe!")
  ...
  scenario "user navigates back to the home page after checking a recipe name" do
    visit '/'
    fill_in 'recipe_name', with: 'chicken noodle soup'
    click_button 'Submit'
    expect(page).to have_content("Try again!")
    click_link 'Try again!'
    expect(page).to have_content("Instructions: Enter a recipe name here")
    expect(page).to have_content("Recipe Name")
    expect(page).to_not have_content("is not a delicious recipe!")
  end
